...
const EMAIL_TEMPLATE_DEFINITIONS = Object.freeze({
  ...
  order_cancelled: {
    ...
    category: 'Orders',
    audience: 'Customer',
    description: 'Confirms that the customer cancellation request was received.',
    defaultSubject: 'Order cancellation received - {{order_number}}',
    defaultBody: `
      <p>Hi {{customer_name}},</p>
      <p>Your request to cancel <strong>{{order_number}}</strong> has been received and the order is now marked cancelled.</p>
      {{{refund_message}}}
    ...
    sampleVariables: {
      customer_name: 'Danielle Brown', order_number: 'FSB-20260729-1042', payment_status: 'Paid',
      refund_message: '<p>Because payment was already collected, our team will review the transaction and process the refund manually. Bank posting times may vary.</p>'
    }
  },
  owner_order_cancelled: {
    name: 'Cancelled order alert',
    category: 'Orders',
    audience: 'Store owner',
    description: 'Alerts the store owner when a customer cancels an order.',
    defaultSubject: 'Order cancelled by customer - {{order_number}}',
    defaultBody: `
      <p>Order <strong>{{order_number}}</strong> was cancelled by the customer.</p>
      <p><strong>Customer:</strong> {{customer_name}} ({{customer_email}})<br><strong>Reason:</strong> {{cancellation_reason}}<br><strong>Payment status:</strong> {{payment_status}}</p>
      {{{refund_action}}}
    `,
    variables: [
      { key: 'order_number', label: 'Order number', description: 'Cancelled order reference.' },
    ...
    sampleVariables: {
      ...
      cancellation_reason: 'Ordered the wrong item', payment_status: 'Paid',
